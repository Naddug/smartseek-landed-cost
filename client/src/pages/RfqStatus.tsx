...
const STATUS_META: Record<string, { labelKey: string; helpKey: string; color: string; icon: React.ReactNode }> = {
  pending:    { labelKey: "rfqStatus.status.pending.label", helpKey: "rfqStatus.status.pending.help", color: "amber",   icon: <Clock className="w-4 h-4" /> },
  routing:    { labelKey: "rfqStatus.status.routing.label", helpKey: "rfqStatus.status.routing.help", color: "blue",    icon: <Search className="w-4 h-4" /> },
  quoted:     { labelKey: "rfqStatus.status.quoted.label", helpKey: "rfqStatus.status.quoted.help", color: "emerald", icon: <CheckCircle2 className="w-4 h-4" /> },
  closed:     { labelKey: "rfqStatus.status.closed.label", helpKey: "rfqStatus.status.closed.help", color: "slate",   icon: <CheckCircle2 className="w-4 h-4" /> },
  needs_info: { labelKey: "rfqStatus.status.needs_info.label", helpKey: "rfqStatus.status.needs_info.help", color: "amber",   icon: <AlertCircle className="w-4 h-4" /> },
};
    ...
    <PublicLayout>
      <section className="bg-gradient-to-b from-slate-950 via-blue-950 to-slate-900 py-14 px-4">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-slate-300 text-sm">{t("rfqStatus.subtitle")}</p>
        </div>
      </section>
          ...
          {data && (
            <div className="mt-6 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                <div>
                  <p className="text-xs text-slate-600 mb-1">{t("rfqStatus.rfq")} #{data.id}</p>
                  <h2 className="text-lg font-bold text-slate-900">{data.productName}</h2>
                  {data.productCategory && <p className="text-xs text-slate-600">{data.productCategory}</p>}
                </div>
                <StatusPill status={data.status} />
              </div>
              <p className="text-xs text-slate-600 mb-4">{t((STATUS_META[data.status] ?? STATUS_META.pending).helpKey)}</p>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex flex-wrap gap-3 text-xs">
                ...
                <Link href="/methodology" className="text-blue-700 underline underline-offset-2">{t("rfqStatus.methodologyLink")}</Link>
              </div>
            </div>
          )}
        </div>
