 ...
 *
 * Serves two purposes:
 *  1. Crawlable by search engines (server injects correct title/meta/JSON-LD)
 *  2. Shows real supplier previews so the page has unique, indexable content
 */
        ...
        <div className="flex items-center justify-between text-xs border-t border-slate-100 pt-2.5">
          ...
          {supplier.employeeCount && (
            <span className="text-slate-400 text-[11px] flex items-center gap-0.5">
              ...
              {supplier.employeeCount.toLocaleString()}+ emp.
            </span>
          )}
        </div>
      </div>
      ...
      >
        <div className="text-center px-6 py-6 max-w-sm">
          ...
          <p className="text-white font-bold text-lg mb-1 leading-tight">
            {(total - 3).toLocaleString()} more {slugToTitle(category).toLowerCase()} suppliers
          </p>
          <p className="text-slate-300 text-sm mb-5 leading-relaxed">
            Free account unlocks 5 results. Pro unlocks all {total.toLocaleString()}.
          </p>
...
export default function SupplierCategoryPage() {
  ...
  // Update document title for client-side navigation (server injects on first load)
  useEffect(() => {
    document.title = `${displayName} Suppliers & Manufacturers – SmartSeek`;
  }, [displayName]);

  // Client-side JSON-LD for ItemList (once suppliers load)
  ...
  // Related categories to show in sidebar / related links
  const related = [
    ...
    <PublicLayout>
      ...
      <section className="bg-gradient-to-b from-slate-950 to-slate-900 pt-20 pb-12 px-4 text-center">
        <div className="max-w-3xl mx-auto">
          {/* Breadcrumb */}
          <nav className="flex justify-center items-center gap-1.5 text-xs text-slate-500 mb-6" aria-label="Breadcrumb">
            <Link href="/" className="hover:text-slate-300 transition-colors">Home</Link>
            <ChevronRight className="w-3 h-3" />
            <Link href="/signup" className="hover:text-slate-300 transition-colors">Suppliers</Link>
            <ChevronRight className="w-3 h-3" />
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-5">
            ...
            {isLoading ? "Loading..." : total > 0 ? `${total.toLocaleString()} verified suppliers found` : "Global Supplier Directory"}
          </div>

          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white leading-tight mb-4">
            {displayName} Suppliers<br />
          ...
          <p className="text-slate-400 text-base sm:text-lg max-w-xl mx-auto mb-8 leading-relaxed">
            Find verified {displayName.toLowerCase()} manufacturers and exporters worldwide.
            Compare certifications, ratings, and contact details — instantly.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-4 text-xs text-slate-500">
            ...
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> 220+ countries</span>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-white font-bold text-lg">
              {isLoading ? "Searching..." : `Top ${displayName} Suppliers`}
            </h2>
            {!isLoading && total > 0 && (
          ...
          {isError && (
            <div className="text-center py-10">
              ...
              <p className="text-slate-400 text-sm">Could not load suppliers right now.</p>
              <Link href="/signup">
                <button className="mt-4 inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold px-5 py-2 rounded-xl text-sm transition">
                  Sign up to search full database <ArrowRight className="w-4 h-4" />
                </button>
              </Link>
            </div>
          )}
          ...
          {!isLoading && !isError && previewSuppliers.length === 0 && (
            <div className="text-center py-14">
              ...
              <p className="text-slate-400 mb-2">No preview results available for this category.</p>
              <Link href="/signup">
                <button className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold px-5 py-2.5 rounded-xl text-sm transition">
                  Search full database <ArrowRight className="w-4 h-4" />
                </button>
              </Link>
            </div>
          )}
      ...
      {/* ── Why SmartSeek / Trust signals ─────────────────────────────────── */}
      <section className="bg-white py-16 px-4 border-t border-slate-100">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold text-slate-900 text-center mb-2">
            Why source {displayName.toLowerCase()} through SmartSeek?
          </h2>
          <p className="text-slate-500 text-sm text-center mb-10 max-w-xl mx-auto">
            SmartSeek aggregates government trade registries, customs records, and third-party verifications
            so you get accurate supplier data — not outdated directories.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              { icon: <ShieldCheck className="w-6 h-6 text-blue-600" />, title: "Government-verified data", desc: "Suppliers cross-referenced with official trade registries across 220+ countries." },
              { icon: <Globe className="w-6 h-6 text-emerald-600" />, title: "Global coverage", desc: "Search manufacturers in China, India, Turkey, Germany, Vietnam and 200+ more markets." },
              { icon: <Search className="w-6 h-6 text-violet-600" />, title: "Instant results", desc: "Natural language search returns hundreds of verified matches in under a second." },
            ].map((f) => (
      ...
      {related.length > 0 && (
        <section className="bg-slate-50 border-t border-slate-100 py-12 px-4">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-lg font-bold text-slate-900 mb-5">Related supplier categories</h2>
      ...
      <section className="bg-slate-950 py-16 px-4 text-center">
        <div className="max-w-lg mx-auto">
          <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">
            Ready to find your ideal<br />{displayName.toLowerCase()} supplier?
          </h2>
          <p className="text-slate-400 text-sm mb-7">
            Join thousands of procurement professionals who trust SmartSeek to find verified suppliers fast.
          </p>
          ...
          <p className="mt-4 text-xs text-slate-600">No credit card · Free plan available · Cancel anytime</p>
        </div>
      </section>
    </PublicLayout>
  );
