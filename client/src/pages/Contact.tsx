...
export default function Contact() {
  ...
  const inquiryTypes = [
    { icon: MessageSquare, label: t("contact.types.sales"), desc: t("contact.types.salesDesc") },
    { icon: HelpCircle, label: t("contact.types.support"), desc: t("contact.types.supportDesc") },
    { icon: Mail, label: t("contact.types.partnerships"), desc: t("contact.types.partnershipsDesc") },
  ];
      ...
      <section className="py-16 bg-slate-50">
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-8 md:p-12 shadow-sm">
            <h2 className="text-xl font-bold text-slate-900 mb-4">{t("contact.sendMessage")}</h2>
            ...
            <p className="text-slate-500 text-sm mt-8">
              {t("contact.browseFirst")} <Link href="/faq" className="text-blue-600 hover:underline font-medium">FAQ</Link> {t("contact.or")} <Link href="/signup" className="text-blue-600 hover:underline font-medium">{t("about.exploreBeta")}</Link>.
            </p>
          </div>

          <div className="mt-16 pt-12 border-t border-slate-200">
            <p className="text-center text-sm text-slate-500 mb-8">{t("contact.dataProtected")}</p>
            <TrustBadges />
          </div>
        </div>
      </section>
