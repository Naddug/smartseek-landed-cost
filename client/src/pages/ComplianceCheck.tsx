      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
          ...
          <CardContent className="space-y-3">
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Supplier Name <span className="text-red-500">*</span></label>
              <input className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none" placeholder="e.g. Wenzhou Neo Electric" value={form.supplierName} onChange={e => setForm({...form, supplierName: e.target.value})} />
            </div>
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Country <span className="text-red-500">*</span></label>
              <input className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none" placeholder="e.g. China" value={form.country} onChange={e => setForm({...form, country: e.target.value})} />
            </div>
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Industry <span className="text-red-500">*</span></label>
              <input className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none" placeholder="e.g. Electronics" value={form.industry} onChange={e => setForm({...form, industry: e.target.value})} />
            </div>
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Target Markets</label>
              <input className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 focus:outline-none" placeholder="USA, EU, UK" value={form.targetMarkets} onChange={e => setForm({...form, targetMarkets: e.target.value})} />
            </div>
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 block">Products</label>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {result && !mutation.isPending && (
            <>
              <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700">
                <CardContent className="pt-6">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
                    <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Compliance Overview</h2>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 mb-4">
                    <div className={`rounded-2xl p-4 sm:p-6 flex flex-col items-center shrink-0 ${result.overallScore >= 80 ? "bg-emerald-50 dark:bg-emerald-950/50" : result.overallScore >= 60 ? "bg-amber-50 dark:bg-amber-950/50" : "bg-red-50 dark:bg-red-950/50"}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-3 w-full bg-slate-200 dark:bg-slate-700 rounded-full h-3">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {result.certifications.map((c, i) => (
                      <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between bg-slate-50 dark:bg-slate-800 rounded-lg p-3 sm:p-4 border border-slate-100 dark:border-slate-700 gap-2">
                        <div className="flex items-center gap-3 min-w-0">
                          <StatusIcon status={c.status} />
                          <div className="min-w-0">
                            <p className="text-sm text-slate-900 dark:text-slate-100 font-semibold truncate">{c.name}</p>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {result.regulatoryChecks.map((r, i) => (
                      <div key={i} className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4 border border-slate-100 dark:border-slate-700">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <StatusIcon status={r.status} />
                            <span className="text-sm text-slate-900 dark:text-slate-100 font-semibold">{r.regulation}</span>
                          </div>
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-300 ml-7 leading-relaxed">{r.description}</p>
              ...
              <Card className={`${result.sanctionsScreening.status === "Clear" ? "border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/30" : "border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-950/30"}`}>
                <CardContent className="pt-5">
                  <div className="flex items-center gap-3 mb-2">
                    <Shield className={`w-6 h-6 ${result.sanctionsScreening.status === "Clear" ? "text-emerald-600" : "text-red-600"}`} />
                    <div>
                      <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">Sanctions Screening</h3>
                  ...
                  <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed mt-2">{result.sanctionsScreening.details}</p>
                </CardContent>
              </Card>
