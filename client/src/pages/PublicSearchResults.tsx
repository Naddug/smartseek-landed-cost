      ...
      <section className="bg-slate-950 py-14 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-6">
            <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Browse Featured Suppliers</h1>
            <p className="text-slate-400 text-sm">Featured suppliers — sign up for full 25.2M+ directory</p>
          </div>
          ...
          <form onSubmit={onSubmit} className="max-w-xl mx-auto mb-8 flex gap-2">
            <input
              ...
              placeholder="Search by company, product, industry, country or city"
              value={input}
          ...
          {!isPending && !isError && (
            <div className="mb-4 text-center">
              <span className="inline-block text-xs text-slate-300 bg-slate-800 border border-slate-700 px-3 py-1 rounded-full">
                {total.toLocaleString()} featured results
              </span>
            </div>
          )}

          {isPending && (
            <div className="text-center py-12 text-slate-400">Loading featured suppliers...</div>
          )}

          {isError && (
            <div className="text-center py-12 text-slate-300">Could not load featured suppliers right now.</div>
          )}
          ...
          {!isPending && !isError && suppliers.length === 0 && (
            <div className="text-center py-14">
              ...
              <p className="text-slate-300 mb-4">No featured suppliers match your search.</p>
              <Link href="/signup">
                <button className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold px-5 py-2.5 rounded-xl text-sm transition">
                  Sign up to access 25.2M+ suppliers <ArrowRight className="w-4 h-4" />
                </button>
              </Link>
            </div>
          )}
