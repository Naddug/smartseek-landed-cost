#!/usr/bin/env node
/**
 * Translate only locales missing home.demo.enterProduct
 */
...
async function main() {
  ...
  const allLocales = readdirSync(LOCALES_DIR).filter((d) => {
    const p = join(LOCALES_DIR, d);
    return statSync(p).isDirectory() && d !== "en";
  });

  const missing = allLocales.filter((locale) => {
    const targetPath = join(LOCALES_DIR, locale, "translation.json");
    const target = JSON.parse(readFileSync(targetPath, "utf8"));
    return !target["home.demo.enterProduct"];
  });

  console.log(`Translating ${Object.keys(source).length} keys to ${missing.length} missing locales: ${missing.join(", ")}`);

  for (const locale of missing) {
    try {
      const targetPath = join(LOCALES_DIR, locale, "translation.json");
      const target = JSON.parse(readFileSync(targetPath, "utf8"));
      const translated = await translateBatch(openai, source, locale);
