...
export const CAMPAIGN_DOCUMENT_REQUIREMENTS: DocumentType[] = [
  "memorandum",
  "financial_statements",
  "use_of_funds_plan",
  "risk_disclosure_signed",
...
export type FinancialDisclosureStandard = {
  ...
  revenue?: { amount: number; period: string; source: string };
  netIncome?: { amount: number; period: string; source: string };
  totalAssets?: { amount: number; asOf: string; source: string };
  totalLiabilities?: { amount: number; asOf: string; source: string };
